...
class CarbonAgent(threading.Thread):
    ...
    def __init__(self, path: str, update_interval: int,
        ...
        #     #df = df.groupby("index").mean(numeric_only=True).reset_index()

        df = pd.read_csv(path)

        # Only downsample if no timestamp exists
        if "timestamp" not in df.columns:
            df["index"] = range(0, df.shape[0])
            df["index"] //= 2
            df = df.groupby("index").mean(numeric_only=True).reset_index()
        
        print(df[["timestamp", "carbon_intensity_avg"]].head())
...
def main():
    ...
    with get_db(args.db_path) as db:
        create_table(db)
        # Auto-register Go controller in cluster
        c = db.cursor()
        c.execute(f"INSERT OR IGNORE INTO {DB_NAME} (app_name, address) VALUES (?, ?)", 
                ("controller", "http://carbon-service-svc"))
        db.commit()
